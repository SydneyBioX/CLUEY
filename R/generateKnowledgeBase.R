...
#' @export
#'
generateKnowledgeBase <- function(exprsMat, celltypes, batch = NULL, method = "ds", minCells = 20){
  if(!(2*ncol(exprsMat) == sum(ncol(exprsMat), length(celltypes)))){
  ...
  idx <- rowSums(exprsMat)
  idx <- which(idx == 0)
  if(length(idx) > 0){
    exprsMat <- suppressWarnings(as.matrix(exprsMat[-idx,]))
  }else{
    exprsMat <- suppressWarnings(as.matrix(exprsMat))
  }
  ...
  if(method == "ds"){
    if(!is.null(batch)){
      ...
      batch_genes <- lapply(batches, function(x){
        ...
        if(length(n_celltypes) > 1){
          ...
          if(length(keep_celltypes) > 0){
            ...
            exprsMat_cepo <- suppressWarnings(Cepo(exprsMat[,idx], celltypes[idx], minCells = minCells, exprsPct = 0.05))
            exprsMat_top_genes <- topGenes(exprsMat_cepo, n = nrow(exprsMat_cepo$stats), returnValues = T)
            ref_genes <- lapply(exprsMat_top_genes, function(x){x[x > 0]})
            ref_genes
          }
      ...
      exprsMat_cepo <- suppressWarnings(Cepo::Cepo(exprsMat, celltypes, minCells = minCells, exprsPct = 0.05))
      exprsMat_top_genes <- Cepo::topGenes(exprsMat_cepo, n = nrow(exprsMat_cepo$stats), returnValues = T)
      ref_genes <- lapply(exprsMat_top_genes, function(x){x[x > 0]})
    }
  ...
  if(method == "de"){
    ...
    seurat_ref <- suppressWarnings(Seurat::CreateSeuratObject(counts = exprsMat, assay = "RNA", meta.data = data.frame(celltypes = celltypes)))
  ...
  exprsMat_pseudobulk <- mapply(function(genes, celltype){
    if(method == "ds"){
      genes <- names(genes)
    }else{
      genes <- intersect(genes, rownames(exprsMat))
    }
    tmp <- exprsMat[genes, which(celltypes == celltype)]
    tmp <- rowMeans(tmp)
    tmp
